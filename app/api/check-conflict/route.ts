// Çakışma kontrolü API - Server-side
  ...
  try {
    ...
    if (action === 'checkProductStatus') {
      ...
      return NextResponse.json({
        isActive: data.status === 'Yayında',
        productName: data.title
      });
    }

    // Tek ürün çakışma kontrolü
    ...
    // Sepet çakışma kontrolü (tüm ürünler)
    if (action === 'checkCartConflicts') {
      ...
      for (const item of cartItems) {
        ...
        const { data: conflictData } = await supabase
          .from('siparisler')
          .select('id')
          .eq('productName', productNameFromDB)
          .in('status', BLOCKING_STATUSES)
          .gte('eventDate', startDate)
          .lte('eventDate', endDate);
