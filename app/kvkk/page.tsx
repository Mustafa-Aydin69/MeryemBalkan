      ...
      <div className={`pt-28 sm:pt-32 pb-20 transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`pb-8 mb-10 border-b transition-colors ${isDarkMode ? 'border-gray-800' : 'border-gray-100'}`}>
            ...
            <h1 className={`text-2xl sm:text-3xl font-light tracking-wide mb-3 transition-colors ${isDarkMode ? 'text-white' : 'text-black'}`}>KİŞİSEL VERİLERİN KORUNMASI (KVKK)</h1>
            <p className={`text-xs transition-colors ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>6698 sayılı Kişisel Verilerin Korunması Kanunu Kapsamında Kişisel Veri Sahibi Hakları</p>
          </div>

          <div className="space-y-10">
            <section>
              <h2 className={sh}>1. VERİ SORUMLUSU</h2>
              <p className={body}>6698 sayılı Kişisel Verilerin Korunması Kanunu ("KVKK") uyarınca, Meryem Balkan Moda Tasarımı veri sorumlusu sıfatıyla, kişisel verilerinizi aşağıda açıklanan kapsamda işleyebilecektir.</p>
            </section>

            <section>
              <h2 className={sh}>2. KİŞİSEL VERİ TÜRLERİ</h2>
              <p className={sub}>Kimlik Verileri</p>
              <ul className="space-y-2 mb-5">
                {['Ad, soyad', 'Doğum tarihi', 'TC Kimlik numarası (gerekli hallerde)', 'Fotoğraf'].map((item) => (
                  <li key={item} className={li}><span className={dot}></span><span>{item}</span></li>
                ))}
              </ul>
              <p className={sub}>İletişim Verileri</p>
              <ul className="space-y-2 mb-5">
                {['Adres bilgileri', 'E-posta adresi', 'Telefon numarası'].map((item) => (
                  <li key={item} className={li}><span className={dot}></span><span>{item}</span></li>
                ))}
              </ul>
              <p className={sub}>Müşteri İşlem Verileri</p>
              <ul className="space-y-2">
                {['Sipariş geçmişi', 'Ödeme bilgileri', 'Randevu kayıtları', 'Ürün tercihleri'].map((item) => (
                  <li key={item} className={li}><span className={dot}></span><span>{item}</span></li>
                ))}
              </ul>
            </section>
            ...
            <section>
              <h2 className={sh}>3. KİŞİSEL VERİLERİN İŞLENME AMAÇLARI</h2>
              <ul className="space-y-2">
                {['Hizmet sunumu ve müşteri memnuniyetinin sağlanması', 'Sözleşme ilişkilerinin kurulması ve ifası', 'Müşteri hizmetleri ve destek süreçlerinin yürütülmesi', 'Pazarlama ve tanıtım faaliyetlerinin gerçekleştirilmesi', 'İstatistik ve analiz çalışmalarının yapılması', 'Yasal yükümlülüklerin yerine getirilmesi', 'İş süreçlerinin planlanması ve yürütülmesi'].map((item) => (
                  <li key={item} className={li}><span className={dot}></span><span>{item}</span></li>
                ))}
              </ul>
            </section>
            ...
            <section>
              <h2 className={sh}>4. VERİ SAHİBİNİN HAKLARI</h2>
              <p className={`${body} mb-3`}>KVKK'nın 11. maddesi uyarınca veri sahipleri olarak sahip olduğunuz haklar:</p>
              <ul className="space-y-2">
                {['Kişisel veri işlenip işlenmediğini öğrenme', 'Kişisel veriler işlenmişse buna ilişkin bilgi talep etme', 'Kişisel verilerin işlenme amacını ve amacına uygun kullanılıp kullanılmadığını öğrenme', 'Yurt içinde veya yurt dışında kişisel verilerin aktarıldığı üçüncü kişileri bilme', 'Kişisel verilerin eksik veya yanlış işlenmiş olması hâlinde bunların düzeltilmesini isteme', 'Kişisel verilerin silinmesini veya yok edilmesini isteme', 'Düzeltme, silme veya yok etme işlemlerinin üçüncü kişilere bildirilmesini isteme', 'Otomatik sistemlerle analiz sonuçlarına itiraz etme', 'Kanuna aykırı işleme sebebiyle zararın giderilmesini talep etme'].map((item) => (
                  <li key={item} className={li}><span className={dot}></span><span>{item}</span></li>
                ))}
              </ul>
            </section>
            ...
            <section>
              <h2 className={sh}>5. HAK KULLANIM YÖNTEMLERİ</h2>
              <p className={`${body} mb-3`}>Yukarıda belirtilen haklarınızı kullanmak için aşağıdaki yöntemlerle başvurabilirsiniz:</p>
              <ul className="space-y-2">
                {['Yazılı başvuru: Atatürk Mahallesi, Muhsin Yazıcıoğlu Caddesi No: 15/B, Merkez / Erzincan', 'E-posta: meryembalkantasarimatolye@gmail.com'].map((item) => (
                  <li key={item} className={li}><span className={dot}></span><span>{item}</span></li>
                ))}
              </ul>
            </section>
            ...
            <section>
              <h2 className={sh}>6. BAŞVURU ŞARTLARI</h2>
              <p className={`${body} mb-3`}>Başvurularınızda aşağıdaki bilgilerin yer alması gerekmektedir:</p>
              <ul className="space-y-2">
                {['Ad, soyad ve başvuru yazılı ise imza', 'Türkiye Cumhuriyeti vatandaşları için T.C. kimlik numarası', 'Tebligata esas yerleşim yeri veya iş yeri adresi', 'Varsa bildirime esas elektronik posta adresi, telefon ve faks numarası', 'Talep konusu'].map((item) => (
                  <li key={item} className={li}><span className={dot}></span><span>{item}</span></li>
                ))}
              </ul>
            </section>
            ...
            <section>
              <h2 className={sh}>7. CEVAPLAMA SÜRESİ</h2>
              <p className={body}>Başvurunuz en geç otuz gün içinde sonuçlandırılacaktır. Başvurunuzun reddedilmesi halinde ret gerekçesi yazılı olarak bildirilecektir.</p>
            </section>

            <section>
              <h2 className={sh}>8. ŞİKAYET HAKKI</h2>
              <p className={body}>Başvurunuzun reddedilmesi, verilen cevabın yetersiz bulunması veya başvurunuza süresinde cevap verilmemesi hallerinde, Kişisel Verileri Koruma Kuruluna şikayette bulunma hakkınız bulunmaktadır.</p>
            </section>

            <section>
              <h2 className={sh}>9. İLETİŞİM</h2>
              <p className={`${body} mb-1`}>KVKK ile ilgili sorularınız için:</p>
